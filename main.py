...
def get_indexes_lower_fps(
    ...
    max_i = np.round(num_frames * new_fps / base_fps)
    new_idx = np.arange(max_i)
    new_idx *= base_fps / new_fps
...
def main(
    ...
    if video_path[video_path.rfind('.') +
                  1:].lower() in cfg['dataset']['file_extensions']:
        ...
        raise FileNotFoundError('Check input file format.')
    ...
    while not_error:
        ...
        for i in range(cfg['model']['batch']):
            ...
            output_image_path = os.path.abspath(
                os.path.join(dir_with_images, f'{counter}.jpg'))
